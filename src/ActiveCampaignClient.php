...
/**
 * Thin wrapper around Laravel's Http client for the ActiveCampaign REST API v3.
 *
...
class ActiveCampaignClient
{
    ...
    protected function request(string $method, string $path, ?array $data = null): array
    {
        $response = Http::withHeaders([
            ...
            ->baseUrl(rtrim($this->apiUrl, '/').'/api/3')
            ->{$method}($path, $data ?? []);

        if ($response->failed()) {
            throw ActiveCampaignException::fromResponse($response);
        }
